...
test.describe("Citizen dashboard and history", () => {
  test.beforeEach(() => {
    if (!isMockMode) test.skip("Navegação/histórico depende de dados previsíveis do mock.");
  ...
  test("citizen can navigate adoption and appointment history", async ({ appPage }) => {
    ...
    await expect(appPage.getByRole("heading", { level: 1, name: /minhas adoções/i })).toBeVisible();
    await appPage.goto("/appointments");
    await expect(appPage.getByRole("heading", { level: 1, name: /meus agendamentos/i })).toBeVisible();
  });
  ...
  test("citizen can navigate catalog and own pets", async ({ appPage }) => {
    ...
    await expect(appPage.getByRole("heading", { level: 1, name: /meus pets/i })).toBeVisible();
  });
});
